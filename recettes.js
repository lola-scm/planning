...
function afficherTableauRecettes(filtre) {
  var recettes = lireRecettes();
  ...
  if (categorie) {
    recettes = recettes.filter(r => r.categorie === categorie);
  }

  var tbody = document.getElementById("rec-tbody");
  if (recettes.length === 0) {
    tbody.innerHTML = "<tr><td colspan='5'>Aucune recette</td></tr>";
    return;
  }
  ...
  for (var i = 0; i < recettes.length; i++) {
    ...
    html += "<tr>";
    html += "<td>" + r.nom + "</td>";
    html += "<td>" + (r.categorie || "-") + "</td>";
    html += "<td>" + (r.portions || 1) + "</td>";
    html += "<td>" + coutPortion + " €</td>";
    html += "<td>";
    ...
    html += "<button class='btn-sm' onclick=\"supprimerRecette('" + r.id + "')\">Supprimer</button>";
    html += "</td>";
    html += "</tr>";
  }
...
function ouvrirFormulaireRecette(recID) {
  ...
  for (var i = 0; i < ingredients.length; i++) {
    ingHtml += ligneIngredient(ingredients[i]);
  }
  ...
  for (var j = 0; j < etapes.length; j++) {
    etapesHtml += ligneEtape(etapes[j]);
  }
...
function ligneIngredient(ing) {
  return "<div class='ing-row'>" +
    ...
    "<input class='form-input' placeholder='Unité (litre, grammes, entier..)' value='" + (ing.unite || "") + "' data-ing='unite' style='width:70px'>" +
    "<input class='form-input' placeholder='Coût €' value='" + (ing.cout || "") + "' data-ing='cout' type='number' step='0.01' style='width:80px'>" +
    "<button class='btn-sm' onclick='supprimerLigne(this)'>✕</button>" +
    "</div>";
}

function ligneEtape(texte) {
  return "<div class='etape-row'>" +
    "<textarea class='form-input' placeholder='Étape…' rows='2'>" + (texte || "") + "</textarea>" +
    "<button class='btn-sm' onclick='supprimerLigne(this)'>✕</button>" +
    "</div>";
}
  ...
  if (idRecetteEnEdition) {
    for (var i = 0; i < recettes.length; i++) {
      if (recettes[i].id == idRecetteEnEdition) {
        ...
        recettes[i].portions = portions; recettes[i].ingredients = ingredients;
        recettes[i].etapes = etapes;
        break;
      }
    }
  } else {
    recettes.push({ id: genererID(), nom: nom, categorie: categorie, portions: portions, ingredients: ingredients, etapes: etapes });
  }
...
function supprimerRecette(recID) {
  if (!confirm("Supprimer cette recette ?")) return;
  sauvegarderRecettes(lireRecettes().filter(r => r.id != recID));
  afficherTableauRecettes();
}


// VOIR RECETTE (modal lecture + ajustement portions)
// ===================================================

function voirRecette(recID) {
  var r = lireRecettes().find(r => r.id == recID);
  if (!r) return;
...
function ajusterPortions() {
  var recID = idRecetteActuelle;
  var r = lireRecettes().find(r => r.id == recID);
  if (!r) return;
...
function fermerVoirRecette() {
  document.getElementById("rec-view-overlay").classList.remove("open");
}


// EXPORT PDF (impression navigateur)
function exporterPDFRecette() {
  window.print();
}
